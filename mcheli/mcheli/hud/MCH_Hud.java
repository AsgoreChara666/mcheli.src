...
public class MCH_Hud extends MCH_BaseInfo
{
    ...
    public void draw(final MCH_EntityAircraft ac, final EntityPlayer player, final float partialTicks) {
        ...
        final ScaledResolution scaledresolution = new W_ScaledResolution(MCH_HudItem.mc, MCH_HudItem.mc.displayWidth, MCH_HudItem.mc.displayHeight);
        MCH_HudItem.scaleFactor = scaledresolution.getScaleFactor();
        if (MCH_HudItem.scaleFactor <= 0) {
            MCH_HudItem.scaleFactor = 1;
        }
        MCH_HudItem.width = MCH_HudItem.mc.displayWidth / (double)MCH_HudItem.scaleFactor;
        MCH_HudItem.height = MCH_HudItem.mc.displayHeight / (double)MCH_HudItem.scaleFactor;
