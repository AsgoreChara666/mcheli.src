...
public class MCH_HudItemString extends MCH_HudItem
{
    ...
    public void execute() {
        final int x = (int)(MCH_HudItemString.centerX + calc(this.posX));
        final int y = (int)(MCH_HudItemString.centerY + calc(this.posY));
        final long dateCount = Minecraft.getMinecraft().thePlayer.worldObj.getTotalWorldTime();
        final int worldTime = (int)((MCH_HudItemString.ac.worldObj.getWorldTime() + 6000L) % 24000L);
        final Date date = new Date();
        final Object[] prm = new Object[this.args.length];
        final double hp_per = (MCH_HudItemString.ac.getMaxHP() > 0) ? (MCH_HudItemString.ac.getHP() / (double)MCH_HudItemString.ac.getMaxHP()) : 0.0;
        for (int i = 0; i < prm.length; ++i) {
            switch (this.args[i]) {
                ...
                case POS_X: {
                    prm[i] = MCH_HudItemString.ac.posX;
                    break;
                }
                case POS_Y: {
                    prm[i] = MCH_HudItemString.ac.posY;
                    break;
                }
                case POS_Z: {
                    prm[i] = MCH_HudItemString.ac.posZ;
                    break;
                }
                case MOTION_X: {
                    prm[i] = MCH_HudItemString.ac.motionX;
                    break;
                }
                case MOTION_Y: {
                    prm[i] = MCH_HudItemString.ac.motionY;
                    break;
                }
                case MOTION_Z: {
                    prm[i] = MCH_HudItemString.ac.motionZ;
                    break;
                }
                case INVENTORY: {
                    prm[i] = MCH_HudItemString.ac.getSizeInventory();
                    break;
                }
                case WPN_NAME: {
                ...
                case ROLL: {
                    prm[i] = MathHelper.wrapAngleTo180_float(MCH_HudItemString.ac.getRotRoll());
                    break;
                }
                case PLYR_YAW: {
                    prm[i] = MCH_Lib.getRotate360(MCH_HudItemString.player.rotationYaw + 180.0f);
                    break;
                }
                case PLYR_PITCH: {
                    prm[i] = -MCH_HudItemString.player.rotationPitch;
                    break;
                }
                case TVM_POS_X: {
