...
public class MCH_HudItemGraduation extends MCH_HudItem
{
    ...
    public void execute() {
        GL11.glPushMatrix();
        final int x = (int)(MCH_HudItemGraduation.centerX + calc(this.drawPosX));
        final int y = (int)(MCH_HudItemGraduation.centerY + calc(this.drawPosY));
        GL11.glTranslated((double)x, (double)y, 0.0);
        GL11.glRotatef((float)calc(this.drawRoll), 0.0f, 0.0f, 1.0f);
        GL11.glTranslated((double)(-x), (double)(-y), 0.0);
        if (this.type == 0) {
            this.drawCommonGraduationYaw(calc(this.drawRot), MCH_HudItemGraduation.colorSetting, x, y);
        }
        else if (this.type == 1) {
            this.drawCommonGraduationPitch1(calc(this.drawRot), MCH_HudItemGraduation.colorSetting, x, y);
        }
        else if (this.type == 2 || this.type == 3) {
            this.drawCommonGraduationPitch2(calc(this.drawRot), MCH_HudItemGraduation.colorSetting, x, y);
        }
        GL11.glPopMatrix();
    }
