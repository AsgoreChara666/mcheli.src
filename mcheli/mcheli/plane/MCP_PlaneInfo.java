...
public class MCP_PlaneInfo extends MCH_AircraftInfo
{
    ...
    public MCP_PlaneInfo(final String name) {
        super(name);
        this.item = null;
        this.nozzles = new ArrayList<MCH_AircraftInfo.DrawnPart>();
    ...
    public void loadItemData(final String item, final String data) {
        ...
        if (item.compareTo("addpartrotor") == 0) {
            ...
            final String[] s = data.split("\\s*,\\s*");
            if (s.length == 6) {
                final MCH_AircraftInfo.DrawnPart n3 = new MCH_AircraftInfo.DrawnPart((MCH_AircraftInfo)this, this.toFloat(s[0]), this.toFloat(s[1]), this.toFloat(s[2]), this.toFloat(s[3]), this.toFloat(s[4]), this.toFloat(s[5]), "nozzle" + this.nozzles.size());
                this.nozzles.add(n3);
            }
        }
    ...
    public class Rotor extends MCH_AircraftInfo.DrawnPart
    {
        ...
        public Rotor(final float x, final float y, final float z, final float rx, final float ry, final float rz, final float mrf, final String model) {
            super((MCH_AircraftInfo)MCP_PlaneInfo.this, x, y, z, rx, ry, rz, model);
    ...
    public class Blade extends MCH_AircraftInfo.DrawnPart
    {
        ...
        public Blade(final int num, final int r, final float px, final float py, final float pz, final float rx, final float ry, final float rz, final String name) {
            super((MCH_AircraftInfo)MCP_PlaneInfo.this, px, py, pz, rx, ry, rz, name);
            this.numBlade = num;
            this.rotBlade = r;
        }
    }
