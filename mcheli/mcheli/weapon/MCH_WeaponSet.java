...
public class MCH_WeaponSet
{
    ...
    public void setRestAllAmmoNum(final int n) {
        ...
        MCH_Lib.DbgLog(this.getFirstWeapon().worldObj, "MCH_WeaponSet.setRestAllAmmoNum:%s %d->%d (%d)", new Object[] { this.getName(), debugBefore, this.numRestAllAmmo, n });
    }
    ...
    public void updateWeapon(final Entity shooter, final boolean isUsed, final int index) {
        ...
        if (isUsed && shooter.worldObj.isRemote && crtWpn != null && crtWpn.cartridge != null) {
            ...
            final float yaw = shooter.rotationYaw;
            final float pitch = shooter.rotationPitch;
        ...
        if (index < this.recoilBuf.length) {
            ...
            if (r.recoilBufCount > 0) {
                if (r.recoilBufCountMax <= 1) {
                    ...
                    final float rb = r.recoilBufCount / (float)r.recoilBufCountMax;
