...
public class MCH_ItemRecipe implements MCH_IRecipeList
{
    ...
    private static void registerCommonItemRecipe() {
        ...
        addRecipeList(addRecipe((Item)itemFuel, MCH_Config.ItemRecipe_Fuel.prmString));
        final MCH_ItemGLTD itemGLTD = MCH_MOD.itemGLTD;
        final MCH_Config config2 = MCH_MOD.config;
        addRecipeList(addRecipe((Item)itemGLTD, MCH_Config.ItemRecipe_GLTD.prmString));
        final MCH_ItemChain itemChain = MCH_MOD.itemChain;
        final MCH_Config config3 = MCH_MOD.config;
        addRecipeList(addRecipe((Item)itemChain, MCH_Config.ItemRecipe_Chain.prmString));
        ...
        addRecipeList(addRecipe((Item)itemContainer, MCH_Config.ItemRecipe_Container.prmString));
        ...
        addRecipeList(addRecipe((Item)itemStinger, MCH_Config.ItemRecipe_Stinger.prmString));
        ...
        addRecipeList(addRecipe((Item)itemStingerBullet, append.append(MCH_Config.ItemRecipe_StingerMissile.prmString).toString()));
        final MCH_ItemLightWeaponBase itemJavelin = MCH_MOD.itemJavelin;
        final MCH_Config config11 = MCH_MOD.config;
        addRecipeList(addRecipe((Item)itemJavelin, MCH_Config.ItemRecipe_Javelin.prmString));
        ...
        addRecipeList(addRecipe((Item)itemJavelinBullet, append2.append(MCH_Config.ItemRecipe_JavelinMissile.prmString).toString()));
        final MCH_ItemSpawnGunner itemSpawnGunnerVsMonster = MCH_MOD.itemSpawnGunnerVsMonster;
        final MCH_Config config13 = MCH_MOD.config;
        addRecipeList(addRecipe(itemSpawnGunnerVsMonster, MCH_Config.ItemRecipe_GunnerVsMonster.prmString));
        final MCH_ItemSpawnGunner itemSpawnGunnerVsPlayer = MCH_MOD.itemSpawnGunnerVsPlayer;
        final MCH_Config config14 = MCH_MOD.config;
        addRecipeList(addRecipe(itemSpawnGunnerVsPlayer, MCH_Config.ItemRecipe_GunnerVsPlayer.prmString));
    ...
    public static void registerItemRecipe() {
        ...
        for (final String name : MCP_PlaneInfoManager.map.keySet()) {
            ...
            addRecipeAndRegisterList(info2, (Item)info2.item, MCP_PlaneInfoManager.getInstance());
        }
        for (final String name : MCH_TankInfoManager.map.keySet()) {
            final MCH_TankInfo info3 = MCH_TankInfoManager.map.get(name);
            addRecipeAndRegisterList(info3, (Item)info3.item, MCH_TankInfoManager.getInstance());
        }
        for (final String name : MCH_VehicleInfoManager.map.keySet()) {
            final MCH_VehicleInfo info4 = MCH_VehicleInfoManager.map.get(name);
            addRecipeAndRegisterList(info4, (Item)info4.item, MCH_VehicleInfoManager.getInstance());
        }
    ...
    public static IRecipe addShapedRecipe(final Item item, final String data) {
        ...
        for (int k = 0; k < r.recipeItems.length; ++k) {
            if (r.recipeItems[k] != null && r.recipeItems[k].getItem() == null) {
    ...
    public static IRecipe addShapelessRecipe(final Item item, final String data) {
        ...
        for (int j = 0; j < r.recipeItems.size(); ++j) {
            final ItemStack is = r.recipeItems.get(j);
            if (is.getItem() == null) {
    ...
    public static ShapelessRecipes getShapelessRecipe(final ItemStack par1ItemStack, final Object... par2ArrayOfObj) {
        ...
        for (int i = par2ArrayOfObj.length, j = 0; j < i; ++j) {
