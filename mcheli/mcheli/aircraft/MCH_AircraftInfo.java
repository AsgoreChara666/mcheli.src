...
public abstract class MCH_AircraftInfo extends MCH_BaseInfo
{
    ...
    public MCH_AircraftInfo(final String s) {
        ...
        this.turretPosition = Vec3.createVectorHelper(0.0, 0.0, 0.0);
    ...
    public boolean isValidData() throws Exception {
        ...
        if (this.isUAV) {
            ...
            if (this.seatList.size() == 0) {
                final MCH_SeatInfo s = new MCH_SeatInfo(Vec3.createVectorHelper(0.0, 0.0, 0.0), false);
                this.seatList.add(s);
            }
        }
    ...
    public void loadItemData(final String item, final String data) {
        if (item.compareTo("displayname") == 0) {
            ...
            if (s != null && s.length >= 7) {
                ...
                this.entityRackList.add(new MCH_SeatRackInfo(names, this.toDouble(s[1]), this.toDouble(s[2]), this.toDouble(s[3]), new CameraPosition(this.toVec3(s[4], s[5], s[6]).addVector(0.0, 1.5, 0.0)), range, para, yaw2, pitch2, rs));
            }
        }
            ...
            if (s.length >= 4) {
                ...
                for (int i = 2; i < s.length; ++i) {
                    this.wheels.add(new Wheel(Vec3.createVectorHelper((double)x, (double)y, (double)this.toFloat(s[i]))));
                }
            ...
            if (!item.equalsIgnoreCase("AddPartSlideRotLG") && s.length >= 6) {
                ...
                if (s.length >= 8) {
                    ...
                    n3.rot2 = Vec3.createVectorHelper((double)this.toFloat(s[7]), (double)this.toFloat(s[8]), (double)this.toFloat(s[9]));
                }
            ...
            if (item.equalsIgnoreCase("AddPartSlideRotLG") && s.length >= 9) {
                ...
                n3.slide = Vec3.createVectorHelper((double)this.toFloat(s[0]), (double)this.toFloat(s[1]), (double)this.toFloat(s[2]));
                this.landingGear.add(n3);
            }
        }
            ...
            if (s.length >= 5) {
                ...
                if (c6.boundingBox.maxY > this.markerHeight) {
                    this.markerHeight = (float)c6.boundingBox.maxY;
                }
                this.markerWidth = (float)Math.max(this.markerWidth, Math.abs(c6.boundingBox.maxX) / 2.0);
                this.markerWidth = (float)Math.max(this.markerWidth, Math.abs(c6.boundingBox.minX) / 2.0);
                this.markerWidth = (float)Math.max(this.markerWidth, Math.abs(c6.boundingBox.maxZ) / 2.0);
                this.markerWidth = (float)Math.max(this.markerWidth, Math.abs(c6.boundingBox.minZ) / 2.0);
                this.bbZmin = (float)Math.min(this.bbZmin, c6.boundingBox.minZ);
                this.bbZmax = (float)Math.min(this.bbZmax, c6.boundingBox.maxZ);
            }
        }
        else if (item.equalsIgnoreCase("RotorSpeed")) {
            this.rotorSpeed = this.toFloat(data, -10000.0f, 10000.0f);
            if (this.rotorSpeed > 0.01) {
                this.rotorSpeed -= (float)0.01;
            }
            if (this.rotorSpeed < -0.01) {
                this.rotorSpeed += (float)0.01;
            }
    ...
    public class Flare
    {
        ...
        public Flare() {
            ...
            this.pos = Vec3.createVectorHelper(0.0, 0.0, 0.0);
        }
    }
    ...
    public class LandingGear extends DrawnPart
    {
        ...
        public LandingGear(final float x, final float y, final float z, final float rx, final float ry, final float rz, final String model, final float maxRotF, final boolean rev, final boolean isHatch) {
            ...
            this.rot2 = Vec3.createVectorHelper(0.0, 0.0, 0.0);
    ...
    public class CameraPosition
    {
        ...
        public CameraPosition(final MCH_AircraftInfo mch_AircraftInfo) {
            this(mch_AircraftInfo, Vec3.createVectorHelper(0.0, 0.0, 0.0));
        }
    }
}
