...
public abstract class MCH_AircraftCommonGui extends MCH_Gui
{
    ...
    public void drawKeyBind(final MCH_EntityAircraft ac, final MCH_AircraftInfo info, final EntityPlayer player, final int seatID, final int RX, final int LX, final int colorActive, final int colorInactive) {
        ...
        final StringBuilder append7 = new StringBuilder().append("Gunner ").append(ac.getGunnerStatus() ? "ON" : "OFF").append(" : ");
        final MCH_Config config9 = MCH_MOD.config;
        final StringBuilder append8 = append7.append(MCH_KeyName.getDescOrName(MCH_Config.KeyFreeLook.prmInt)).append(" + ");
        ...
        if (seatID >= 0 && seatID <= 1 && ac.haveFlare()) {
            ...
            final StringBuilder append9 = new StringBuilder().append("Flare : ");
            final MCH_Config config11 = MCH_MOD.config;
            msg = append9.append(MCH_KeyName.getDescOrName(MCH_Config.KeyFlare.prmInt)).toString();
            this.drawString(msg, RX, this.centerY - 50, c);
        }
        if (seatID == 0 && info.haveLandingGear()) {
            if (ac.canFoldLandingGear()) {
                final StringBuilder append10 = new StringBuilder().append("Gear Up : ");
                final MCH_Config config12 = MCH_MOD.config;
                msg = append10.append(MCH_KeyName.getDescOrName(MCH_Config.KeyGearUpDown.prmInt)).toString();
                this.drawString(msg, RX, this.centerY - 40, colorActive);
            }
            else if (ac.canUnfoldLandingGear()) {
                final StringBuilder append11 = new StringBuilder().append("Gear Down : ");
                final MCH_Config config13 = MCH_MOD.config;
                msg = append11.append(MCH_KeyName.getDescOrName(MCH_Config.KeyGearUpDown.prmInt)).toString();
                this.drawString(msg, RX, this.centerY - 40, colorActive);
            }
        }
        final MCH_WeaponSet ws = ac.getCurrentWeapon((Entity)player);
        if (ac.getWeaponNum() > 1) {
            final StringBuilder append12 = new StringBuilder().append("Weapon : ");
            final MCH_Config config14 = MCH_MOD.config;
            msg = append12.append(MCH_KeyName.getDescOrName(MCH_Config.KeySwitchWeapon2.prmInt)).toString();
            this.drawString(msg, LX, this.centerY - 70, colorActive);
        }
        if (ws.getCurrentWeapon().numMode > 0) {
            final StringBuilder append13 = new StringBuilder().append("WeaponMode : ");
            final MCH_Config config15 = MCH_MOD.config;
            msg = append13.append(MCH_KeyName.getDescOrName(MCH_Config.KeySwWeaponMode.prmInt)).toString();
            this.drawString(msg, LX, this.centerY - 60, colorActive);
        }
        if (ac.canSwitchSearchLight((Entity)player)) {
            final StringBuilder append14 = new StringBuilder().append("SearchLight : ");
            final MCH_Config config16 = MCH_MOD.config;
            msg = append14.append(MCH_KeyName.getDescOrName(MCH_Config.KeyCameraMode.prmInt)).toString();
            this.drawString(msg, LX, this.centerY - 50, colorActive);
        }
        else if (ac.canSwitchCameraMode(seatID)) {
            final StringBuilder append15 = new StringBuilder().append("CameraMode : ");
            final MCH_Config config17 = MCH_MOD.config;
            msg = append15.append(MCH_KeyName.getDescOrName(MCH_Config.KeyCameraMode.prmInt)).toString();
            this.drawString(msg, LX, this.centerY - 50, colorActive);
        }
        if (seatID == 0 && ac.getSeatNum() >= 1) {
            int color = colorActive;
            if (info.isEnableParachuting && MCH_Lib.getBlockIdY(ac, 3, -10) == 0) {
                final StringBuilder append16 = new StringBuilder().append("Parachuting : ");
                ...
                final StringBuilder append17 = new StringBuilder().append("Repelling : ");
                final MCH_Config config19 = MCH_MOD.config;
                msg = append17.append(MCH_KeyName.getDescOrName(MCH_Config.KeyUnmount.prmInt)).toString();
                color = -256;
            }
            else {
                final StringBuilder append18 = new StringBuilder().append("Dismount : ");
                final MCH_Config config20 = MCH_MOD.config;
                msg = append18.append(MCH_KeyName.getDescOrName(MCH_Config.KeyUnmount.prmInt)).toString();
            }
        ...
        if ((seatID == 0 && ac.canSwitchFreeLook()) || (seatID > 0 && ac.canSwitchGunnerModeOtherSeat(player))) {
            final StringBuilder append19 = new StringBuilder().append("FreeLook : ");
            final MCH_Config config21 = MCH_MOD.config;
            msg = append19.append(MCH_KeyName.getDescOrName(MCH_Config.KeyFreeLook.prmInt)).toString();
